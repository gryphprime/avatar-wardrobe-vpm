...
(function(global){
  ...
  function normalize(type,input,context,id,predecessor){
    ...
    if(!['wear-outfit','replace-outfit','remove-outfit','prepare-preview','capture-source','render-snapshot','undo-operation'].includes(type))throw new Error('Unsupported operation.');
    input=input||{};
    var payload={};
    ...
    if(type!=='render-snapshot'&&type!=='prepare-preview'&&type!=='capture-source'&&type!=='undo-operation'&&!/^[a-f0-9]{32}$/i.test(payload.variantId||''))throw new Error('Choose a specific outfit variant first.');
    if(type==='undo-operation'&&!/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/.test(payload.undoToken||''))throw new Error('This session-only Undo is no longer available.');
  ...
  function create(options){
    ...
    function dismiss(id){
      var record=records.get(id);if(!record||!terminal.has(record.state))throw new Error('Only a settled change can be dismissed. Cancel pending work instead.');
    ...
    function retry(id,scopeId){
      ...
      if(!retryAvailable(original))return Promise.reject(new Error('Review this change first. Uncertain or accepted changes must keep their original request ID.'));
      var action=(async function(){
        if(!enabled)throw new Error('Reconnect the desktop host before retrying.');
        if(scopeId&&scopeId!==original.command.target.scopeId)throw new Error('Choose the original preset before retrying this change.');
        if(!context||key(Object.assign({},context,{scopeId:original.command.target.scopeId}))!==key(original.command.target))throw new Error('Choose the original pinned avatar before retrying this change.');
        ...
        if(!receipt||receipt.id!==id||receipt.state!=='failed'||!receipt.command||JSON.stringify(receipt.command)!==JSON.stringify(original.command))throw new Error('The durable result changed or is uncertain. Review the current avatar before doing anything else.');
        if(!next||!next.revision||!context||key(Object.assign({},next,{scopeId:original.command.target.scopeId}))!==key(original.command.target)||key(Object.assign({},context,{scopeId:original.command.target.scopeId}))!==key(original.command.target))throw new Error('The pinned project, scene or avatar changed. Review the outfit on the current target.');
        if(ordered().some(function(record){return record.command&&record.command.target.projectId===next.projectId&&mutations.has(record.type||record.command.type)&&!terminal.has(record.state);}))throw new Error('Wait for pending outfit changes to settle before retrying.');
