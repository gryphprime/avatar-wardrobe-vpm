...
(function(global){
  ...
  function create(options){
    ...
    function draw(){
      actions.replaceChildren();review.replaceChildren();if(!summary){if(!busy)button('Refresh',function(){load();});return;}
      button(summary.hasSaved?'Replace saved appearance':'Save current appearance',save,!summary.revision);
      if(!summary.revision)button('Refresh',function(){load();});
    ...
    async function load(inspecting){cancelInspection();if(!inspecting)inspectionAttempts=0;var ticket=++serial;busy=true;draw();try{var value=await request('',null,false);if(!valid(ticket))return;summary=value;status.textContent=(value.hasSaved?'Saved '+new Date(value.savedAt).toLocaleString()+' · '+value.garments+' garments · '+value.materials+' material slots · '+value.shapes+' shapes. ':'')+value.message;if(value.hasSaved)status.textContent+=' Saving again replaces this preset’s saved appearance.';if(!value.revision)status.textContent='Inspecting the avatar before saving… '+(inspectionAttempts>=30?'Use Refresh when Unity is ready.':'');}catch(error){if(valid(ticket)){summary=null;status.textContent=error.message;}}finally{if(valid(ticket)){busy=false;draw();scheduleInspection(ticket);}}}
    async function save(){if(busy||!summary||!summary.revision)return;cancelInspection();var ticket=++serial;busy=true;pending=null;draw();status.textContent='Saving appearance references…';try{await request('_save','&revision='+encodeURIComponent(summary.revision),true);if(!valid(ticket))return;await load();if(options.onChange)options.onChange();}catch(error){if(valid(ticket)){status.textContent=error.message;busy=false;draw();}}}
    async function restore(){if(busy)return;cancelInspection();var ticket=++serial;busy=true;pending=null;draw();status.textContent='Checking exact copies and asset versions…';try{var result=await request('_review',null,true);if(valid(ticket)){pending=result;status.textContent='The working avatar is unchanged. Review the changes below.';}}catch(error){if(valid(ticket))status.textContent=error.message;}finally{if(valid(ticket)){busy=false;draw();}}}
