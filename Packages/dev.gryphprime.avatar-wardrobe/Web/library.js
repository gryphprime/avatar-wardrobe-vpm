...
(function(global){
  ...
  function paint(){
    ...
    shown.forEach(function(item){
      ...
      item.usage.forEach(function(use){var li=document.createElement('li');li.textContent=[use.project,use.avatar,use.instance].filter(Boolean).join(' / ');names.appendChild(li);});
      ...
      row.append(heading,detail,state,variants,action,impact);list.appendChild(row);
    });
  }
  ...
  async function add(input){
    ...
    try{
      for(var file of files){status(T('library.adding',file.name));var result=await request('/api/library/add?filename='+encodeURIComponent(file.name),{method:'POST',headers:{'X-Wardrobe-Request':'1'},body:file});status(result.duplicate?T('library.duplicate'):T('library.added'));}
      await refresh();
