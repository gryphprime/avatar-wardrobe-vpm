...
(function(global){
  ...
  async function refresh(){try{var result=await request('/api/library');items=result.items;var page=result.items;while(page.length===100){var next=await request('/api/library?offset='+items.length);page=next.items;items=items.concat(page);}R.text(el('libraryProject'),result.project||T('library.noProject'));paint();}catch(error){status(error.message);}}
