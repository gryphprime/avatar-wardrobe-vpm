...
function setup(fetcher){
 const context={window:{},document:{getElementById:()=>null},fetch:fetcher,TextEncoder};
 vm.runInNewContext(source,context);return context.window.WardrobeReporting;
}
(async()=>{
 let calls=[];const api=setup(async(url,options)=>{calls.push({url,options});return {ok:true,status:200,json:async()=>({id:'confirmed'})};});
 ...
 await assert.rejects(()=>setup(async()=>({ok:true,status:200,json:async()=>({})})).send(bug),/did not confirm/);
