...
function setup(fetcher){
 const context={TextEncoder,window:{},document:{getElementById:()=>null},fetch:fetcher};
 vm.runInNewContext(source,context);return context.window.WardrobeReporting;
}
(async()=>{
 let calls=[];const api=setup(async(url,options)=>{calls.push({url,options});return {ok:true,status:201,json:async()=>({id:'confirmed'})};});
 ...
 assert.equal(api.itemMetadata({}, {phys:Infinity,contact:-1}).physicsComponents,0);assert.equal(api.itemMetadata({}, {phys:Infinity,contact:-1}).contacts,0);
 ...
 await assert.rejects(()=>api.send({message:'字'.repeat(23000)}),/too large/);
 ...
 await assert.rejects(()=>setup(async()=>({ok:true,status:201,json:async()=>({})})).send(bug),/did not confirm/);
